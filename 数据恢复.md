...
binlog 是二进制文件，需用 mysqlbinlog 工具转换为可读的 SQL 语句，筛选出备份后到误删前的有效操作。
...
编辑 binlog.sql，删除误操作（如 delete、drop）及其之后的语句：

```bash
# vim /root/binlog.sql
```

如果不确定如何删除 binlog.sql 里的内容，可以记下 Delete_rows 的时间节点，然后只导出该时间节点之前的binlog：

```bash
# mysqlbinlog --database test --start-datetime="2026-01-05 00:00:00" --stop-datetime="2026-01-05 12:00:00" /root/mysql-bin.000021 > /root/binlog.sql
```

注：

1. 如果能确定误操作的时间节点，就可以只导出该时间节点之前的binlog，也就无需再编辑导出之后的binlog.sql：

   ```bash
   # mysqlbinlog --database test --start-datetime="2026-01-05 00:00:00" --stop-datetime="2026-01-05 12:00:00" /root/mysql-bin.000021 > /root/binlog.sql
   ```

2. 如果要输出伪 sql 和注释，可以使用参数 ```--base64-output=DECODE-ROWS -vv```，***重点：执行该参数导出的 sql 无法恢复数据***：

   ```bash
   # mysqlbinlog --base64-output=DECODE-ROWS -vv --database test --start-datetime="2026-01-05 00:00:00" --stop-datetime="2026-01-05 12:00:00" /root/mysql-bin.000021 > /root/binlog.sql
   ```
...
```bash
# mysql -h127.0.0.1 -uroot -proot --database test < /usr/local/mysql/backup/test_20260105.sql
```

补充 binlog 中的新增数据：

```bash
# mysql -h127.0.0.1 -uroot -proot --database test < /root/binlog.sql
```
