...
- LIKE '%word%' 查询不会使用索引，也就是索引会失效，但是 LIKE 'word%' 查询使用索引
...
- 类型不一致会导致索引失效，比如字段 age 是 int 类型的，使用 WHERE age = '20' ， 则会导致索引失败，应该改为 WHERE age = 20
...
|关键字|描述|
|--|--|
|SIMPLE|对于不包含子查询和其他复杂语法的简单查询，这是一个常 见的类型。|
|PRIMARY|这是为更复杂的查询而创建的首要表(也就是最外层的表)。这个类型通常可以在DERIVED 和UNION 类型混合使用时见到。|
|DERIVED|当一个表不是一个物理表时，那么就被叫做DERIVED。|
...
|关键字|描述|
|--|--|
|ALL|全表扫描，对数据表从头到尾查找一遍|
|INDEX|全索引扫描，对索引从头到尾查找一遍|
|RANGE|表示所有符合一个给定范围值的索引行都被用到|
|INDEX_MERGE|合并索引，使用多个单列索引搜索|
|REF|表示所有具有匹配的索引值的行都被用到|
...
- possible_keys
 ...
 查询数据记录时实际选用的索引，这个索引按字节计算的长度在key_len数据列里给出。比如说，对于一个INTEGER数据列的索引，这个字节长度将是4。如果用到了复合索引，在key_len数据列里还可以看到MySQL具体使用了它的哪些部分。作为一般规律，key_len数据列里的值越小越好(意思是更快)
 
- key_len
...
执行查询时预计会从这个数据表里读出的数据行数，只是预估值

- filtered

百分比的值，这个百分比值和rows 列的值一起使用，可以估计出那些将要和 QEP 中的前一个表进行连接的行的数目。
...
|Using temporary|使用了内部临时(基于内存的)表。|
